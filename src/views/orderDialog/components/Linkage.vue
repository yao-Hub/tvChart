<template>
  <div class="linkage">
    <div v-for="(value, key) in state.form" :key="key" class="item">
      <div
        class="linkageInput"
        :style="{ backgroundColor: ifError(key, value) ? '#9f4747' : '' }"
      >
        <a-input
...
watch(
  ...
  (newVal) => {
    ...
    state.form.point =
      state.pointCache || (props.stopType === "stopLoss" ? "-100" : "100");
  }
);
...
watch(
  ...
  () => {
    ...
    for (const i in state.form) {
      if (i !== state.focusKey) {
        state.form[i] =
          state.calculateAction[i as keyof typeof state.calculateAction]();
      }
    }
  },
  {
...
function calculatePoint() {
  ...
  const subtractedPrice =
    props.transactionType === "buy" ? -currentBuy : +currentSell;
